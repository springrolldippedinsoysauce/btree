...
public class TwoThreeFourTree implements java.io.Serializable
{ 
    //private class fields for 234-Tree
    private BlockNode root;
    private int size;
    private int height;
    //private class fields that sets the rules for a 234-Tree
    ...
    *Default Constructor 
    *Import: none
    *Export: A 234-Tree with a fixed size of order 4
    *Assertion: This constructor makes a new 234-Tree.
    **************************************************************************/
    public TwoThreeFourTree ()
    {
        root = null;//Starts with an empty 234-Tree
        size = 0;//New 234-Tree with no nodes inserted
        height = 0;//Will increment when splitting upwards
        this.minKeySize = order;//2
        this.minChildSize = minKeySize + 1; //3
        this.maxKeySize = 2 * minKeySize; //4
        this.maxChildSize = maxKeySize + 1; //5
    }
    ...
    *Assertion: This constructor makes a new 234-Tree.
    **************************************************************************/
    public TwoThreeFourTree (String filename)
    {
        root = null;//Starts with an empty 234-Tree
        size = 0;//New 234-Tree with no nodes inserted
        height = 0;//Will increment when splitting upwards
    ...
    *Import: none
    *Export: size (int)
    *Assertion: Returns the total number of nodes inserted into 234-Tree
    **************************************************************************/
    public int getSize ()
    {
        return size;
    ...
    *Assertion: Returns the ideal height of the 234-Tree
    **************************************************************************/
    ...
    *Export: none
    *Assertion: Responsible for the insertion function of the 234-Tree, acts as
    *           a wrapper method for insertRecursion
    **************************************************************************/
    public void insert(String key, Object value)
    {
        //If the 234-Tree is empty, insert it as the new root
        if(root == null)
        {
    ...
    **************************************************************************/
    private void insertRecursion (BlockNode current, String key, Object value)
    {
        ...
        {
            /****************************************************************
            *If it encounters a full node, split regardless as per 234-Tree *
            *splitting behaviour.                                           *
            ****************************************************************/
            //Zero-based indexing array
            if(current.getKeySize() == maxKeySize - 1)
            ...
            else
            {
                while(!tieBreaker)
                {
                    ...
                    //Indexing moves the pointer to traverse the 234-Tree
                    index++;    
                }
            }
        }
    ...
    *           of the 234-Tree. Only split when the node is full.
    **************************************************************************/
    private void split(BlockNode nodeToSplit)
    {
        ...
        if(node.parent == null)
        {
            //New root, increases height of the 234-Tree
        ...
        else
        {
            ...
            //Remove the node that has been split
    ...
    *           cases in which the key is located the in 234-Tree. Returns an 
    ...
    *Import: key (String)
    *Export: none
    *Assertion: No plans for implementation of deletion function for 234-Tree 
